...
"""
        ...
        self.lambda_flatfield = Float(
            ...
            value=0,
            doc="""\
            If you set the flat-field regularization parameter to 0 or a negative value, an estimated value is used. 
            We recommend to use internally etimated value. High values (eg. 9.5) increase thespatial regularization 
            strength, yielding a more smooth flatfield. A default value estimated from input images.
            """
        )
        ...
        self.lambda_darkfield = Float(
            ...
            value=0,
            doc="""\
            If you set the dark-field regularization parameter to 0 or a negative value, an estimated value is used. 
            We recommend to use internally etimated value. High values (eg. 9.5) increase thespatial regularization 
            strength, yielding a more smooth flatfield.  A default value estimated from input images.
            """
        )
...
#            self.reweight_tolerance 
        ...
        if len(image_numbers) > 0:
            ...
            # Find the module that provides the image we need
                ...
                for w in pipeline.run_group_with_yield(
                    ...
                    output_image_provider.list_images(image)
    ...
    def run(self, workspace):
        ...
        if self.when_calculate == LAST_CYCLE:
            #
            ...
            output_image_provider.list_images(orig_image)
            output_image_provider.serialize(d)

        workspace.image_set.providers.append(output_image_provider)
        flatfield = output_image_provider.provide_flatfield(workspace.image_set)
        darkfield = output_image_provider.provide_darkfield(workspace.image_set)
        baseline_drift = output_image_provider.provide_baseline_drift(workspace.image_set)
        ...
        if self.show_window:
            ...
            if self.if_baseline_drift.value is True:
                workspace.display_data.baseline_drift = baseline_drift.pixel_data
    ...
    def post_group(self, workspace, grouping):
        ...
        assert isinstance(output_image_provider, BasictIlluminationImageProvider)
...
class BasicIlluminationImageProvider(AbstractImage):
    ...
    def __init__(self, flatfield_image_name, darkfield_image_name, module, pybasic_settings):
        ...
        self.__if_baseline_drift = pybasic_settings['if_baseline_drift']
        self.__if_pybasic_not_ran = False
        self.__images_list = []
        self.__cached_flatfield = None
        self.__cached_darkfield = None
        self.__cached_baseline_drift = None
        self.__pybasic_settings = pybasic_settings
#        self.__cached_stacked_image = None
#        self.__cached_dilated_image = None
#        self.__cached_mask_count = None

    D_FLATFIELD = "flatfield"
    D_DARKFIELD = "darkfield"
    ...
    def serialize(self, d):
        ...
        """
        d[self.D_FLATFIELD] = self.__flatfield_image_name
        d[self.D_DARKFIELD] = self.__darkfield_image_name
...
#        d[self.D_MASK_COUNT] = self.__mask_count
    ...
    def deserialize(d, module):
        ...
        provider = BasicIlluminationImageProvider(
            d[BasicIlluminationImageProvider.D_FLATFIELD], 
            d[BasicIlluminationImageProvider.D_DARKFIELD],
            module,
            d[BasicIlluminationImageProvider.D_PYBASIC_SETTINGS]
        )
        provider.__if_pybasic_not_ran = True
    ...
    def list_images(self, image):
        ...
        image - an instance of cellprofiler.cpimage.Image, including
                image data
        """
        self.__if_pybasic_not_ran = True
        self.__images_list.append(image.pixel_data)

    def provide_background(self, image_set):
        if self.__if_pybasic_not_ran:
            self.run_pybasic()
    ...
    def provide_flatfield(self, image_set):
        if self.__if_pybasic_not_ran:
            self.run_pybasic()
        return self.__cached_flatfield

    def provide_darkfield(self, image_set):
        if self.__if_pybasic_not_ran:
            self.run_pybasic()
        return self.__cached_darkfield

    def provide_baseline_drift(self, image_set):
        if self.__if_pybasic_not_ran:
            self.run_pybasic()
        return self.__cached_baseline_drift

    def run_pybasic(self):
        _flatfield, _darkfield = pybasic(
            images_list = self.__images_list,
            **self.__pybasic_settings
        )
        if self.__if_baseline_drift:
            _drift = baseline_drift(
                images_list = self.__images_list,
                flatfield = _flatfield,
                darkfield = _darkfield,
                working_size = self.__pybasic_settings['working_size'],
            )
            self.__cached_baseline_drift = Image(np.reshape(_drift, (-1,1)) )

        
        self.__cached_flatfield = Image(_flatfield)
        self.__cached_darkfield = Image(_darkfield)
        self.__if_pybasic_not_ran = False
...
def pybasic(
    ...
    ''' Because there is no example for segmentation, the segmentation option is ignored
